...
Private Const COLLECTION_LIMIT_FIAT_BUY As Long = 500
Private Const COLLECTION_LIMIT_FIAT_SELL As Long = 500
Private Const COLLECTION_LIMIT_FIAT_DEPOSIT As Long = 500
Private Const COLLECTION_LIMIT_FIAT_WITHDRAW As Long = 500
...
' Import all assets currencies from Asset table and make them Keep-on-Compact
'
Public Sub BinanceTool_Currencies_Export()
    Dim table As ListObject
    Set table = ThisWorkbook.Sheets(SHEET_CURRENCIES).ListObjects(TABLE_CURRENCIES)
    
    Dim col As ListColumn
    Set col = table.ListColumns(COL_CURRENCIES_TICKER)
    
    Dim cell As range
    For Each cell In col.DataBodyRange
        Dim ws As Worksheet
        Set ws = findWorksheet(cell.value, True)
        
        ws.Activate
    Next cell
End Sub
'
' Remove unneeded currencies with respect with Keep-on-Compact column values
'
Public Sub BinanceTool_Currencies_Compact()

    Dim currenciesTable As ListObject
    Set currenciesTable = ThisWorkbook.Sheets(SHEET_CURRENCIES).ListObjects(TABLE_CURRENCIES)

    Dim keepOnCompactCol As ListColumn
    Set keepOnCompactCol = currenciesTable.ListColumns(COL_CURRENCIES_KEEP_ON_COMPACT)

    Dim i As Long
    Dim cell As range

    If Not keepOnCompactCol.DataBodyRange Is Nothing Then
        For i = currenciesTable.ListRows.count To 1 Step -1
            Set cell = keepOnCompactCol.DataBodyRange(i)
            If Not cell.value Then
                currenciesTable.ListRows(i).Delete
            End If
        Next i
    End If
    
    Dim tickerCol As ListColumn
    Set tickerCol = currenciesTable.ListColumns(COL_CURRENCIES_TICKER)
    
    For i = currenciesTable.ListColumns.count To CURRENCIES_FIRST_TRADE_COLUMN_INDEX Step -1
        Dim col As ListColumn
        Set col = currenciesTable.ListColumns(i)
        
        Set cell = tickerCol.DataBodyRange.Find(what:=col.name, LookIn:=xlValues, LookAt:=xlWhole, MatchCase:=False)
    ...
    If currenciesTable.ListColumns.count < CURRENCIES_FIRST_TRADE_COLUMN_INDEX Then
        formula = "=0"
    Else
        formula = "=COUNTIF(" & TABLE_CURRENCIES & "[@[" & currenciesTable.ListColumns(CURRENCIES_FIRST_TRADE_COLUMN_INDEX).name & "]:[" & currenciesTable.ListColumns(currenciesTable.ListColumns.count).name & "]],TRUE)"
    End If
    
    Dim tradesCol As ListColumn
    Set tradesCol = currenciesTable.ListColumns(COL_CURRENCIES_SYMBOLS)
    
    For Each cell In tradesCol.DataBodyRange
        cell.formula = formula
    Next cell
End Sub
'
' Collect all currencies info
'
Public Sub BinanceTool_Currencies_Collect()
    '
    ' Collect All Coins Infornation
    '
    Dim coins As collection
    Set coins = BinanceApi_Wallet_GetAllCoinsInfo()
    
    If coins Is Nothing Then
        Exit Sub
    End If
    ...
    For Each coin In coins
        ...
        '
        ' If coin has no row in Currencies table then adds new row
        '
        If tickerCol.DataBodyRange Is Nothing Then
            currenciesTable.ListRows.Add
            rowNum = currenciesTable.ListRows.count
            tickerCol.DataBodyRange(rowNum).value = ticker
        Else
            Set cell = tickerCol.DataBodyRange.Find(what:=ticker, LookIn:=xlValues, LookAt:=xlWhole, MatchCase:=False)
        ...
        '
        ' Creates record for every coin
        '
        Set record = New Dictionary
        Set trades = New Dictionary
        
        record.Add COL_CURRENCIES_NAME, coin("name")
        record.Add COL_CURRENCIES_TYPE, IIf(coin("isLegalMoney"), "Fiat", "Crypto")
        record.Add COL_CURRENCIES_BASES, 0
        record.Add COL_CURRENCIES_QUOTES, 0
        record.Add COL_CURRENCIES_TRADES, trades
        record.Add COL_CURRENCIES_PRECISION, IIf(coin("isLegalMoney"), 2, 8)
        
        data.Add ticker, record
    Next coin
    
    '
    ' Collect Exchange Information - specificly All Trading Symbols
    '
    Dim exchangeInfo As Dictionary
    Set exchangeInfo = BinanceApi_SpotTrading_exchangeInfo(False)
    
    If exchangeInfo Is Nothing Then
        Exit Sub
    End If
    ...
    '
    ' For every trading symbol:
    ' update existing base  asset record - increments "bases"
    ' update existing base  asset record - in "trades" dictionary adds <quoteAsset>=false
    ' update existing quote asset record - increments "quotes"
    '
    Dim symbol As Dictionary
    For Each symbol In symbols
        ticker = symbol("baseAsset")
        If data.Exists(ticker) Then
            Set record = data(ticker)
            record(COL_CURRENCIES_BASES) = record(COL_CURRENCIES_BASES) + 1
            
            If record(COL_CURRENCIES_PRECISION) > symbol("baseAssetPrecision") Then
                record(COL_CURRENCIES_PRECISION) = symbol("baseAssetPrecision")
            End If
            
            ticker = symbol("quoteAsset")
            If data.Exists(ticker) Then
                record(COL_CURRENCIES_TRADES).Add ticker, False
                
                Set record = data(ticker)
                record(COL_CURRENCIES_QUOTES) = record(COL_CURRENCIES_QUOTES) + 1
            
                If record(COL_CURRENCIES_PRECISION) > symbol("quoteAssetPrecision") Then
                    record(COL_CURRENCIES_PRECISION) = symbol("quoteAssetPrecision")
                End If
            End If
        End If
    Next symbol
    ...
    Dim key As Variant
    '
    ' For every ticker with quotes adds new column (if not already added)
    '
    For Each key In data.Keys
        Set record = data(key)
        
        If record(COL_CURRENCIES_QUOTES) > 0 Then
            For i = CURRENCIES_FIRST_TRADE_COLUMN_INDEX To currenciesTable.ListColumns.count
                ...
                End If
            Next i
            If i > currenciesTable.ListColumns.count Then
                Set col = currenciesTable.ListColumns.Add(currenciesTable.ListColumns.count + 1)
                col.name = key
            End If
        End If
    Next key
    ...
    '
    ' For every table row fill all info
    '
    Dim formula As String
    If currenciesTable.ListColumns.count < CURRENCIES_FIRST_TRADE_COLUMN_INDEX Then
        formula = ""
    ...
    For Each cell In tickerCol.DataBodyRange
        rowNum = cell.row - currenciesTable.HeaderRowRange.row
        
        key = cell.value
        
        If Not data.Exists(key) Then
            currenciesTable.ListColumns(COL_CURRENCIES_BASES).DataBodyRange(rowNum).value = 0
            currenciesTable.ListColumns(COL_CURRENCIES_QUOTES).DataBodyRange(rowNum).value = 0
            currenciesTable.ListColumns(COL_CURRENCIES_SYMBOLS).DataBodyRange(rowNum).value = 0
        
            For i = CURRENCIES_FIRST_TRADE_COLUMN_INDEX To currenciesTable.ListColumns.count
                Set col = currenciesTable.ListColumns(i)
                col.DataBodyRange(rowNum).value = " "
            Next i
        Else
            Set record = data(key)
        
            currenciesTable.ListColumns(COL_CURRENCIES_NAME).DataBodyRange(rowNum).value = record(COL_CURRENCIES_NAME)
            currenciesTable.ListColumns(COL_CURRENCIES_TYPE).DataBodyRange(rowNum).value = record(COL_CURRENCIES_TYPE)
            currenciesTable.ListColumns(COL_CURRENCIES_BASES).DataBodyRange(rowNum).value = record(COL_CURRENCIES_BASES)
            currenciesTable.ListColumns(COL_CURRENCIES_QUOTES).DataBodyRange(rowNum).value = record(COL_CURRENCIES_QUOTES)
            currenciesTable.ListColumns(COL_CURRENCIES_PRECISION).DataBodyRange(rowNum).value = record(COL_CURRENCIES_PRECISION)
            currenciesTable.ListColumns(COL_CURRENCIES_FORMAT).DataBodyRange(rowNum).formula = "=Format[Test Value]"
            currenciesTable.ListColumns(COL_CURRENCIES_SYMBOLS).DataBodyRange(rowNum).formula = formula
            
            For i = CURRENCIES_FIRST_TRADE_COLUMN_INDEX To currenciesTable.ListColumns.count
                Set col = currenciesTable.ListColumns(i)
                If Not record(COL_CURRENCIES_TRADES).Exists(col.name) Then
                    col.DataBodyRange(rowNum).value = " "
                End If
            Next i
        End If
...
'
'
Public Sub BinanceTool_Assets_UpdateDistributions()
    Dim operation As Dictionary
    Dim Operations As Dictionary
    Set Operations = New Dictionary
    ...
    lastUpdateTime = IIf(filter = "", getLastUpdate(COL_LAST_UPDATE_DISTRIBUTION), CDate("2025-06-01"))
    ...
    Do
        ...
        Do
            Dim data As Dictionary
            Set data = BinanceApi_Wallet_GetDividendHistory(filter, startTime, endTime, COLLECTION_LIMIT_DIVIDEND)
            ...
            Dim item As Dictionary
            For Each item In list
                opTime = UnixTimestamp2Date(item("divTime"))
                ...
                operation(COL_DETAILS_STAMP) = opTime
                operation(COL_DETAILS_WALLET) = WALLET_SPOT
                operation(COL_DETAILS_OPERATION) = OPERATION_DISTRIBUTION
                operation(COL_DETAILS_ACQUIRED) = item("amount")
                ...
                operation(COL_DETAILS_CHARGE) = 0
                operation(COL_DETAILS_BNB_FEE) = 0
                operation(COL_DETAILS_NOTE) = item("enInfo")
                operation(COL_DETAILS_ID) = ID_PREFIX_DISTRIBUTION & item("id")
                
                Operations(ticker).Add operation
            Next item
            
            If list.count < COLLECTION_LIMIT_DIVIDEND Then
                endTime = DateAdd("s", -1, startTime)
                Exit Do
            Else
                endTime = DateAdd("s", -1, endTime)
...
'
'
Public Sub BinanceTool_Assets_UpdateCryptoDeposits()
    Dim operation As Dictionary
    Dim Operations As Dictionary
    Set Operations = New Dictionary
    ...
    lastUpdateTime = getLastUpdate(COL_LAST_UPDATE_CRYPTO_DEPOSIT)
    ...
    Do
        ...
        startTime = DateAdd("d", -90, endTime)
        ...
        Do
            Dim list As collection
            Set list = BinanceApi_Wallet_GetDepositHistory(True, "", startTime, endTime, COLLECTION_LIMIT_CRYPTO_DEPOSIT)
            ...
            Dim item As Dictionary
            For Each item In list
                opTime = UnixTimestamp2Date(item("completeTime"))
                ...
                If item("status") = 1 Then
                    ...
                    operation(COL_DETAILS_STAMP) = opTime
                    operation(COL_DETAILS_WALLET) = getWalletByType(item("walletType"))
                    operation(COL_DETAILS_OPERATION) = OPERATION_DEPOSIT
                    operation(COL_DETAILS_ACQUIRED) = item("amount")
                    operation(COL_DETAILS_SPENT) = 0
                    operation(COL_DETAILS_TICKER) = ""
                    operation(COL_DETAILS_PRICE) = 0
                    ...
                    operation(COL_DETAILS_CHARGE) = 0
                    operation(COL_DETAILS_BNB_FEE) = 0
                    operation(COL_DETAILS_NOTE) = item("network") & ":" & item("sourceAddress")
                    operation(COL_DETAILS_ID) = ID_PREFIX_CRYPTO_DEPOSIT & item("id")
                    
                    Operations(ticker).Add operation
                End If
            Next item
            
            If list.count < COLLECTION_LIMIT_CRYPTO_DEPOSIT Then
                endTime = DateAdd("s", -1, startTime)
                Exit Do
            Else
                endTime = DateAdd("s", -1, endTime)
    ...
    Call setLastUpdate(COL_LAST_UPDATE_CRYPTO_DEPOSIT)
End Sub
'
'
'
Public Sub BinanceTool_Assets_UpdateCryptoWithdraws()
    Dim operation As Dictionary
    Dim Operations As Dictionary
    Set Operations = New Dictionary
    ...
    lastUpdateTime = getLastUpdate(COL_LAST_UPDATE_CRYPTO_WITHDRAW)
    ...
    Do
        ...
        Do
            Dim list As collection
            Set list = BinanceApi_Wallet_GetWithdrawHistory("", startTime, endTime, COLLECTION_LIMIT_CRYPTO_WITHDRAW)
            ...
            For Each item In list
                ...
                If item("status") = 6 Then
                    ...
                    operation(COL_DETAILS_STAMP) = opTime
                    operation(COL_DETAILS_WALLET) = getWalletByType(item("walletType"))
                    operation(COL_DETAILS_OPERATION) = OPERATION_WITHDRAW
                    operation(COL_DETAILS_ACQUIRED) = 0
                    operation(COL_DETAILS_SPENT) = item("amount")
                    operation(COL_DETAILS_TICKER) = ticker
                    operation(COL_DETAILS_PRICE) = 0
                    operation(COL_DETAILS_AMOUNT) = 0
                    operation(COL_DETAILS_CHARGE) = item("transactionFee")
                    operation(COL_DETAILS_BNB_FEE) = 0
                    operation(COL_DETAILS_NOTE) = item("network") & ":" & item("address")
                    operation(COL_DETAILS_ID) = ID_PREFIX_CRYPTO_WITHDRAW & item("id")
                    
                    Operations(ticker).Add operation
                End If
            Next item
            
            If list.count < COLLECTION_LIMIT_CRYPTO_WITHDRAW Then
                endTime = DateAdd("s", -1, startTime)
                Exit Do
            Else
                endTime = DateAdd("s", -1, endTime)
    ...
    Call setLastUpdate(COL_LAST_UPDATE_CRYPTO_WITHDRAW)
End Sub
'
'
'
Public Sub BinanceTool_Assets_UpdateFiatDeposits()
    Dim operation As Dictionary
    Dim Operations As Dictionary
    Set Operations = New Dictionary
    ...
    lastUpdateTime = getLastUpdate(COL_LAST_UPDATE_FIAT_DEPOSIT)
    ...
    Do
        ...
        Do
            Dim data As Dictionary
            Set data = BinanceApi_Fiat_GetDeposits(startTime, endTime, COLLECTION_LIMIT_FIAT_DEPOSIT)
            ...
            For Each item In list
                ...
                If item("status") = "Successful" Then
                    ...
                    operation(COL_DETAILS_WALLET) = WALLET_SPOT
                    operation(COL_DETAILS_OPERATION) = OPERATION_DEPOSIT
                    operation(COL_DETAILS_ACQUIRED) = item("indicatedAmount")
                    operation(COL_DETAILS_SPENT) = 0
                    operation(COL_DETAILS_TICKER) = ticker
                    operation(COL_DETAILS_PRICE) = 0
                    operation(COL_DETAILS_AMOUNT) = 0
                    operation(COL_DETAILS_CHARGE) = item("totalFee")
                    operation(COL_DETAILS_BNB_FEE) = 0
                    operation(COL_DETAILS_NOTE) = item("method")
                    operation(COL_DETAILS_ID) = ID_PREFIX_FIAT_DEPOSIT & item("orderNo")
                    
                    Operations(ticker).Add operation
                End If
            Next item
            
            If list.count < COLLECTION_LIMIT_FIAT_DEPOSIT Then
                endTime = DateAdd("s", -1, startTime)
                Exit Do
            Else
                endTime = DateAdd("s", -1, endTime)
    ...
    Call setLastUpdate(COL_LAST_UPDATE_FIAT_DEPOSIT)
End Sub
'
'
'
Public Sub BinanceTool_Assets_UpdateFiatWithdraws()
    Dim operation As Dictionary
    Dim Operations As Dictionary
    Set Operations = New Dictionary
    ...
    lastUpdateTime = getLastUpdate(COL_LAST_UPDATE_FIAT_WITHDRAW)
    ...
    Do
        ...
        Do
            Dim data As Dictionary
            Set data = BinanceApi_Fiat_GetWithdraws(startTime, endTime, COLLECTION_LIMIT_FIAT_WITHDRAW)
            ...
            For Each item In list
                ...
                If item("status") = "Successful" Then
                    ...
                    operation(COL_DETAILS_BNB_FEE) = 0
                    operation(COL_DETAILS_NOTE) = item("method")
                    operation(COL_DETAILS_ID) = ID_PREFIX_FIAT_WITHDRAW & item("orderNo")
                    
                    Operations(ticker).Add operation
                End If
            Next item
            
            If list.count < COLLECTION_LIMIT_FIAT_WITHDRAW Then
                endTime = DateAdd("s", -1, startTime)
                Exit Do
            Else
                endTime = DateAdd("s", -1, endTime)
    ...
    Call setLastUpdate(COL_LAST_UPDATE_FIAT_WITHDRAW)
End Sub
'
'
'
Public Sub BinanceTool_Assets_UpdateFiatBuys()
    Dim operation As Dictionary
    Dim Operations As Dictionary
    Set Operations = New Dictionary
    ...
    lastUpdateTime = getLastUpdate(COL_LAST_UPDATE_FIAT_BUY)
    ...
    Do
        ...
        Do
            Dim data As Dictionary
            Set data = BinanceApi_Fiat_GetBuys(startTime, endTime, COLLECTION_LIMIT_FIAT_BUY)
            ...
            For Each item In list
                ...
                If item("status") = "Completed" Then
                    ...
                    operation(COL_DETAILS_WALLET) = WALLET_SPOT
                    operation(COL_DETAILS_OPERATION) = OPERATION_FIAT_BUY
                    operation(COL_DETAILS_ACQUIRED) = item("sourceAmount")
                    operation(COL_DETAILS_SPENT) = 0
                    operation(COL_DETAILS_TICKER) = ticker
                    operation(COL_DETAILS_PRICE) = 0
                    operation(COL_DETAILS_AMOUNT) = 0
                    operation(COL_DETAILS_CHARGE) = item("totalFee")
                    operation(COL_DETAILS_BNB_FEE) = 0
                    operation(COL_DETAILS_NOTE) = TEXT_NA
                    operation(COL_DETAILS_ID) = ID_PREFIX_FIAT_BUY & item("orderNo")
                    ...
                    operation(COL_DETAILS_OPERATION) = OPERATION_CONVERT_OUT
                    operation(COL_DETAILS_ACQUIRED) = 0
                    operation(COL_DETAILS_SPENT) = Str2Dec(item("sourceAmount")) - Str2Dec(item("totalFee"))
                    operation(COL_DETAILS_TICKER) = item("cryptoCurrency")
                    operation(COL_DETAILS_PRICE) = Round(Str2Dec(item("obtainAmount")) / (Str2Dec(item("sourceAmount")) - Str2Dec(item("totalFee"))), 8)
                    operation(COL_DETAILS_AMOUNT) = item("obtainAmount")
                    operation(COL_DETAILS_CHARGE) = 0
                    operation(COL_DETAILS_BNB_FEE) = 0
                    operation(COL_DETAILS_NOTE) = "FIAT:BUY"
                    operation(COL_DETAILS_ID) = ID_PREFIX_CONVERT_OUT & item("orderNo")
                    ...
                    operation(COL_DETAILS_WALLET) = WALLET_SPOT
                    operation(COL_DETAILS_OPERATION) = OPERATION_CONVERT_IN
                    operation(COL_DETAILS_ACQUIRED) = item("obtainAmount")
                    operation(COL_DETAILS_SPENT) = 0
                    operation(COL_DETAILS_TICKER) = item("fiatCurrency")
                    operation(COL_DETAILS_PRICE) = item("price")
                    operation(COL_DETAILS_AMOUNT) = Str2Dec(item("sourceAmount")) - Str2Dec(item("totalFee"))
                    operation(COL_DETAILS_CHARGE) = 0
                    operation(COL_DETAILS_BNB_FEE) = 0
                    operation(COL_DETAILS_NOTE) = "FIAT:BUY"
                    operation(COL_DETAILS_ID) = ID_PREFIX_CONVERT_IN & item("orderNo")
                    
                    Operations(ticker).Add operation
                End If
            Next item
            
            If list.count < COLLECTION_LIMIT_FIAT_BUY Then
                endTime = DateAdd("s", -1, startTime)
                Exit Do
            Else
                endTime = DateAdd("s", -1, endTime)
    ...
    Call setLastUpdate(COL_LAST_UPDATE_FIAT_BUY)
End Sub
'
'
'
Public Sub BinanceTool_Assets_UpdateFiatSells()
    Dim operation As Dictionary
    Dim Operations As Dictionary
    Set Operations = New Dictionary
    ...
    lastUpdateTime = getLastUpdate(COL_LAST_UPDATE_FIAT_SELL)
    ...
    Do
        ...
        Do
            Dim data As Dictionary
            Set data = BinanceApi_Fiat_GetSells(startTime, endTime, COLLECTION_LIMIT_FIAT_SELL)
            ...
            For Each item In list
                ...
                If item("status") = "Completed" Then
                    ...
                    operation(COL_DETAILS_OPERATION) = OPERATION_CONVERT_OUT
                    operation(COL_DETAILS_ACQUIRED) = 0
                    operation(COL_DETAILS_SPENT) = item("obtainAmount")
                    operation(COL_DETAILS_TICKER) = item("fiatCurrency")
                    operation(COL_DETAILS_PRICE) = item("price")
                    operation(COL_DETAILS_AMOUNT) = Str2Dec(item("sourceAmount")) + Str2Dec(item("totalFee"))
                    operation(COL_DETAILS_CHARGE) = 0
                    operation(COL_DETAILS_BNB_FEE) = 0
                    operation(COL_DETAILS_NOTE) = "FIAT:SELL"
                    operation(COL_DETAILS_ID) = ID_PREFIX_CONVERT_OUT & item("orderNo")
                    ...
                    operation(COL_DETAILS_WALLET) = WALLET_SPOT
                    operation(COL_DETAILS_OPERATION) = OPERATION_CONVERT_IN
                    operation(COL_DETAILS_ACQUIRED) = Str2Dec(item("sourceAmount")) + Str2Dec(item("totalFee"))
                    operation(COL_DETAILS_SPENT) = 0
                    operation(COL_DETAILS_TICKER) = item("cryptoCurrency")
                    operation(COL_DETAILS_PRICE) = Round(Str2Dec(item("obtainAmount")) / (Str2Dec(item("sourceAmount")) + Str2Dec(item("totalFee"))), 8)
                    operation(COL_DETAILS_AMOUNT) = item("obtainAmount")
                    ...
                    operation(COL_DETAILS_STAMP) = opTime
                    operation(COL_DETAILS_WALLET) = WALLET_SPOT
                    operation(COL_DETAILS_OPERATION) = OPERATION_FIAT_SELL
                    operation(COL_DETAILS_ACQUIRED) = 0
                    operation(COL_DETAILS_SPENT) = item("sourceAmount")
                    operation(COL_DETAILS_TICKER) = ticker
                    operation(COL_DETAILS_PRICE) = 0
                    operation(COL_DETAILS_AMOUNT) = 0
                    operation(COL_DETAILS_CHARGE) = item("totalFee")
                    operation(COL_DETAILS_BNB_FEE) = 0
                    operation(COL_DETAILS_NOTE) = TEXT_NA
                    operation(COL_DETAILS_ID) = ID_PREFIX_FIAT_SELL & item("orderNo")
    ...
    Call setLastUpdate(COL_LAST_UPDATE_FIAT_SELL)
End Sub
'
' Collect all assets in funding wallet
'
Private Sub BinanceTool_Assets_CollectFundingAssets(ByVal Wallet As Dictionary, ByVal name As String)
    Dim assets As collection
    Set assets = BinanceApi_Wallet_GetFundingAsset()
    ...
    For Each asset In assets
        ...
        If Not Wallet.Exists(ticker) Then
            Wallet.Add ticker, New Dictionary
        End If
        
        amount = Str2Dec(asset("free")) + Str2Dec(asset("locked")) + Str2Dec(asset("freeze")) + Str2Dec(asset("withdrawing"))
        
        Wallet(ticker)(name) = amount
    Next asset
End Sub
'
Private Sub BinanceTool_Assets_CheckAsset(ByVal ticker As String, Optional ByVal amounts As Dictionary = Nothing)
    Dim table As ListObject
    Set table = ThisWorkbook.Sheets(SHEET_ASSETS).ListObjects(TABLE_ASSETS)
    
    Dim col As ListColumn
    Set col = table.ListColumns(COL_ASSETS_TICKER)
    
    Dim cell As range
    If col.DataBodyRange Is Nothing Then
        Set cell = Nothing
    Else
        Set cell = col.DataBodyRange.Find(what:=ticker, LookIn:=xlValues, LookAt:=xlWhole, MatchCase:=False)
    End If
    ...
    Else
        rowNum = table.ListRows.Add().Index
        
        col.DataBodyRange(rowNum).value = ticker
    End If
    
    If Not amounts Is Nothing Then
        Dim name As Variant
        For Each name In amounts.Keys
            table.ListColumns(name).DataBodyRange(rowNum).value = amounts(name)
        Next name
    End If
End Sub
'
